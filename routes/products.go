...
import (
  "fmt"
  "net/http"
  "go-psql-demo-webapp/utils"
  "go-psql-demo-webapp/models"
  "go-psql-demo-webapp/sessions"
  "strconv"
  "log"
  "errors"
  "html"
...
var (
  ...
  ErrRequiredProductName = errors.New("Name of product is required")
)
...
func productCreatePostHandler(w http.ResponseWriter, r *http.Request) {
  ...
  sessions.Message("New product added successfully!", "success", r, w)
...
func productDeleteGetHandler(w http.ResponseWriter, r *http.Request) {
  ...
  sessions.Message(fmt.Sprintf("%d product was deleted permanently.", rows), "warning", r, w)
  http.Redirect(w, r, "/products", 302)
}
